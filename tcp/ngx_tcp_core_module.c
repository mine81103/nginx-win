...
static ngx_command_t  ngx_tcp_core_commands[] = {
    ...
    // NOTE for TCP_PROXY: NGX_CONF_MULTI not supported in this version of nginx
    { ngx_string("server"),
...
#ifdef _WIN32 // TCP_PROXY
        if (ngx_memcmp(ls[i].sockaddr + off, &u.sockaddr.sockaddr + off, len) != 0) {
            continue;
        }
#else
